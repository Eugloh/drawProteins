...
## Drawing complete translated precursors

For a processed protein, UniProt's `CHAIN` can describe the mature product
rather than the complete translated sequence. The opt-in precursor extent
draws only the visual backbone from residue 1 through the true translated
sequence length. It does not renumber or modify any UniProt feature coordinate.

For example, ADAM10 has SIGNAL and PROPEP annotations before its mature CHAIN.
Those annotations explain observations in precursor coordinates that occur
before the mature product:

```{r adam10_precursor, eval=FALSE}
...
The SIGNAL, PROPEP, CHAIN, and DOMAIN rows keep their full-sequence UniProt
coordinates. The default `draw_chains(p, features)` remains the annotated
CHAIN rendering for backward compatibility.
